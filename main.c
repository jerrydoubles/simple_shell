	...
	for (int i = 0; info->arg[i]; i++)
{
	...
	cmd_path = find_command_path(info, get_env_var(info, "PATH="), info->argv[0]);
	...
	if (cmd_path)
{
	...
	{
if ((is_interactive(info) || get_env_var(info, "PATH=") || info->argv[0][0] == '/') && is_valid_cmd(info, info->argv[0]))
	{
	fork_and_execute_cmd(info);
	}
	else if (*(info->arg) != '\n')
